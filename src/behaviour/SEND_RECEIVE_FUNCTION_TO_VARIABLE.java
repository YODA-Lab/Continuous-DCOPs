...
public class SEND_RECEIVE_FUNCTION_TO_VARIABLE extends OneShotBehaviour {
  ...
  public void action() {
    ...
    System.out.println("Agent " + agent.getIdStr() + " at iteration " + agent.getLsIteration() + " choose the best value: " + bestValue);
  ...
  private double calculateTheBestValue() {
    ...
    MaxSumMessage msg = new MaxSumMessage(agent.getMSvalueSet());
  ...
  private MaxSumMessage createFUNC_TO_VAR(PiecewiseMultivariateQuadFunction function, MaxSumMessage var2FuncMsg, AID functionAgent, int toSendOrStore) {
    ...
    double agentToProject;
    Set<Double> agentToProjectValueSet = null;
    
    if (toSendOrStore == FUNC_TO_VAR_TO_SEND_OUT) {
      agentToKeepValueSet = agent.getMSvalueSet();
      agentToKeep = Double.valueOf(agent.getIdStr());
      
      agentToProjectValueSet = var2FuncMsg.getNewValueSet();
      agentToProject = Double.valueOf(functionAgent.getLocalName());
    }
    else { // if (toSendOrStore == FUNC_TO_VAR_TO_STORE) {
      agentToKeep = Double.valueOf(functionAgent.getLocalName());
      agentToKeepValueSet = var2FuncMsg.getNewValueSet();

      agentToProject = Double.valueOf(agent.getIdStr());
      agentToProjectValueSet = agent.getMSvalueSet();
    }
    ...
    List<Double> agentLabel = new ArrayList<>();
    ...
    for (List<Double> listValueEntry : setOfScopeAgentValues) {
      ...
      mapValue.put(String.valueOf((int) agentToKeep), agentToKeepValue);
      mapValue.put(String.valueOf((int) agentToProject), agentToProjectValue);
    ...
    Table projectedTable = behaviour.DPOP_UTIL.projectOperator(discretizedFunction, agentToProject);
